...
## STEP 5-5 — 구조 검증 체크리스트

파일 쓰기 전 권장안(verdict="권장") 기준으로 아래 항목을 검증한다. FAIL 항목이 있으면 해당 필드를 보완한 후 STEP 6으로 진행한다.

| # | 검증 항목 | 기준 | FAIL 시 처리 |
|---|---|---|---|
| 1 | 솔루션명 존재 | solution_name이 비어있지 않음 | 권고안에서 추출하여 보완 |
| 2 | 솔루션 ID 존재 | solution_id가 MS 제품 조합 형식 (예: PowerAutomate+SharePointList) | 권고안에서 추출하여 보완 |
| 3 | 리스크 섹션 존재 | risks[]가 존재하고 최소 1개 이상의 리스크 포함 | 리스크 1개 이상 생성하여 보완 |
| 4 | 판정 존재 | verdict가 "권장" \| "검토필요" \| "비추천" 중 하나 | 권고안 판정 재확인 |
| 5 | 구현 개요 충분 | implementation[]이 3단계 이상 | 단계를 세분화하여 보완 |

**검증 결과 출력** (터미널, 파일에는 포함하지 않음):
```
[구조 검증] ✅ PASS (5/5)
```
또는
```
[구조 검증] ⚠️ FAIL — #3 리스크 섹션 누락 → 보완 후 진행
```
