...
| 스킬 | 버전 | 역할 | 타입 |
|---|---|---|---|
| `consult` | v1.3 | 전체 흐름 오케스트레이터 | project-specific |
| `parse-requirement` | v4.0 | 요구사항 구조화 + session_id 생성 + 언어 감지 | project-specific |
| `ai-score-compare` | v3.2 | 솔루션 자유 제안 + 리스크 2패스 평가 + 요약 출력 | project-specific |
| `ms-solution-recommend` | v3.0 | solutions.md 검증 데이터 참고 제공 (Deep 전용) | project-specific |
| `generate-output` | v1.4 | 최종 산출물 파일 생성 (ko/en/en+ko) | project-specific |
...
| `archive` | v1.0 | 이력 보존 정책 (4주 보존 + CSV + Cold Storage) | general |
...
[consult] ─────────────────────────────────────────────────────
    ...
    │
    ├─ STEP 6 ────► 사용자 최종 확인 + 산출물 형식 선택 + 언어 선택
    ...
    └─ STEP 7 ────► [generate-output]
                         ...
                         produces: output/YYYYMMDD_[도메인]_[session_id]_자동화_컨설팅[_EN|_BILINGUAL].txt
```
...
| 항목 | 내용 |
|---|---|
| **트리거** | "컨설팅 시작해줘", "자동화 방법 알려줘", "업무 자동화 컨설팅" |
| **입력** | 사용자 자유 형식 요구사항 |
| **호출 스킬** | parse-requirement → ai-score-compare → ai-analysis → generate-output |
| **reads** | `.claude/skills/consult/references/reconsult-guide.md` |
| **출력** | 최종 산출물 파일 경로 + 완료 보고 |
| **신규 (v1.2)** | 적합성 게이트 STEP 1-5 (proceed/partial/rejected 3단계 분기) |
| **신규 (v1.3)** | STEP 5-1-5 Evidence Summary 압축 (WebSearch 원문 → 4개 필드) |

---

### parse-requirement (v4.0)
...
| **reads** | `.claude/skills/parse-requirement/references/parsing-guide.md` |
| **출력** | ParsedRequirement 구조체 + `logs/session/session_YYYYMMDD_NNN.json` |
| **신규 (v4.0)** | 입력 언어 자동 감지 (ko/en). 세션 파일에 input_language 저장 |

---

### ai-score-compare (v3.2)

| 항목 | 내용 |
|---|---|
| **트리거** | consult STEP 3에서 모드 선택 후 자동 실행 / 재컨설팅 시 재실행 |
| **입력** | ParsedRequirement + mode (quick/deep) + scope_limited 컨텍스트 |
| **호출 스킬** | ms-solution-recommend (Deep 모드만) / ai-analysis |
| **reads** | `.claude/skills/ai-score-compare/references/blocklist.md` / `risk-evaluation-guide.md` |
| **출력** | SolutionProposal 목록 (기본: 요약 포맷 / 상세보기 요청 시: 전체 형식) |
| **신규 (v3.2)** | 기본 출력 요약화. "[N]안 상세보기" 트리거로 전체 형식 출력 |
...
### ms-solution-recommend (v3.0)

| 항목 | 내용 |
|---|---|
| **트리거** | ai-score-compare Deep 모드에서 내부 호출 (Quick에서는 절대 호출 안 함) |
| **입력** | ParsedRequirement |
| **호출 스킬** | 없음 |
| **reads** | `.claude/skills/ms-solution-recommend/references/blocklist.md` / `solutions.md` |
| **출력** | solutions_context + blocklist_context → ai-score-compare에 반환 |
| **역할** | solutions.md의 검증 데이터를 참고용으로만 제공. 솔루션 선택 제한 없음 |

---

### generate-output (v1.4)

| 항목 | 내용 |
|---|---|
| **트리거** | consult STEP 7에서 사용자 최종 확인 후 호출 |
| **입력** | session_id, mode, SolutionProposal 목록, output_mode, output_language |
| **호출 스킬** | 없음 |
| **reads** | `logs/ai_analysis/ai_analysis_YYYYMMDD.jsonl` (REVISION 이벤트, session_id 필터) |
| **출력** | `output/YYYYMMDD_[도메인]_[session_id]_자동화_컨설팅[_EN\|_BILINGUAL][_user\|_developer].txt` |
| **신규 (v1.4)** | output_language: ko/en/en+ko. en+ko 시 영문 본문 + 한국어 번역 단일 파일 |
...
| `archive/YYYYMM_summary.csv` | archive | 사용자 직접 열람 |
